...
def get_user_settings(username):
    ...
    if not user:
        return {}
    user_settings = model_to_dict(user, exclude=[
        'id',
        'added',
        'is_admin',
...
def get_users_in_server(server_id, permissions = []):
    ...
    relev_users = User.select().where((User.id << users_in_server) | (User.id << unrestricted_users))
...
def get_user_requests_last_24_hours(username):
    ...
    recent_requests = MediaRequest.select().join(UserRequests).join(User).where(
        (MediaRequest.created >= one_day_ago) &
        (User.id == user.id)
    )
