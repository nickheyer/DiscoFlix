...
class ContentSelectionView(discord.ui.View):
    ...
    async def manage_trailer_button(self, is_created):
        if is_created and not self.trailer_button:
            self.trailer_button = discord.ui.Button(label='Trailer', style=discord.ButtonStyle.secondary, custom_id='trailer', row=1)
    ...
    async def update_embed(self, interaction=False, timed_out=False):
        ...
        embed.title += f' ({content["year"]})' if 'year' in content and str(content['year']) not in content.get('title', '') else ''
        ...
        if seasons:
            ...
            if (not self.user_config.is_admin and
                (self.config.max_seasons_for_non_admin != 0 and
                seasons > self.config.max_seasons_for_non_admin)):
        ...
        for key, value in content.items():
            if key in relevant_fields.keys():
                embed.insert_field_at(index=0, name=key.title(), value=str(value).title() + relevant_fields[key], inline=True)
...
class ApproveNewUser(discord.ui.View):
    ...
    @discord.ui.button(label="Register User", style=discord.ButtonStyle.blurple, custom_id="register_user", row=0)
    ...
    @discord.ui.button(label="Register Admin", style=discord.ButtonStyle.green, custom_id="register_admin", row=0)
    async def register_admin_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user not in self.authorized_users:
            return
    ...
    @discord.ui.button(label="Deny", style=discord.ButtonStyle.gray, custom_id="deny", row=0)
    async def select_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user not in self.authorized_users:
            return
        self.result = 'DENIED'
    ...
    async def generate_embed(self, interaction=False, timed_out=False):
        if not self.authorized_users:
            embed = discord.Embed(
                ...
                color=discord.Color.brand_red())
            embed.description = ('Authorization is required for this action, but no admin users exist in this server.\n\n' +
...
class ApproveRequest(discord.ui.View):
    ...
    @discord.ui.button(label="Approve", style=discord.ButtonStyle.green, custom_id="approve", row=0)
    async def register_admin_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user not in self.authorized_users:
            return
        self.result = True
        await interaction.response.edit_message(content=None, view=None, embed=self.original_embed)
        self.stop()
    
    @discord.ui.button(label="Deny", style=discord.ButtonStyle.red, custom_id="deny", row=0)
    async def select_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user not in self.authorized_users:
            return
        self.result = False
