...
async function createServer(data = {}) {
  const server = await prisma.discordServer.create({ data });
  return server;
}

async function getServer(where = {}) {
  const server = await prisma.discordServer.findFirst({ where });
  return server;
}

async function getServers(where = {}) {
  const servers = await prisma.discordServer.findMany({ where });
  return servers;
}
