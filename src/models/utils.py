...
def get_user_from_id(id):
    ...
    user_dict = model_to_dict(
        user, backrefs=True, recurse=True, manytomany=True, max_depth=1
    )
...
def edit_user_from_dict(data):
    ...
    for server in user.discord_servers:
        if server.server_id not in list(
            map(lambda x: x.get("server_id", None), server_list)
        ):
            server.users.remove(user)
    for server in server_list:
        new_server, created = DiscordServer.get_or_create(server_id=server["server_id"])
